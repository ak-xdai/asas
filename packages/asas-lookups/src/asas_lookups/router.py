...
def build_routers(get_session: Callable) -> Routers:
    ...
    def create_type(payload: LookupTypeCreate, session: Session = Depends(get_session)):
        ...
        # An unknown scope never reaches here — the schema types the field as
        # TypeScope, so request validation answers 422 with the allowed values.
        if payload.is_open and payload.scope is not TypeScope.org:
